          ...
          {loading ? (
            ...
            <div className={
              ...
              {projects.map((project, index) => {
                ...
                const isLightBg = colorClass.includes('white') || colorClass.includes('neutral-50');
                ...
                return viewMode === 'list' ? (
                    ...
                    <div className="flex-1 p-6 bg-white flex items-center justify-between min-w-0">
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        {project.sectors && project.sectors.length > 0 && (
                          <span className="rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium text-neutral-700 whitespace-nowrap">
                            {project.sectors[0]}
                          </span>
                        )}
                  ...
                  >
                    ...
                    {/* Content Overlay */}
                    <div className={`relative h-full flex flex-col justify-between p-6 ${isLightBg ? 'text-black' : 'text-white'}`}>
                      {/* Top Section */}
                      <div>
                        <h3 className="text-2xl font-bold mb-2">{project.title}</h3>
                        <p className="text-sm opacity-90">{project.summary || 'No description available'}</p>
                      </div>
                        ...
                        {project.participants && project.participants.length > 0 && (
                          <div className="mb-4">
                            ...
                            <ul className="space-y-0.5">
                              {project.participants.slice(0, 4).map((member, i) => (
                                <li key={i} className="text-sm opacity-90">• {member.name}</li>
                              ))}
                            </ul>
                          </div>
                        )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {selectedPerson.projects.map((project, i) => (
                      <button
                        ...
                        className="border border-neutral-200 rounded-lg p-4 hover:shadow-md transition text-left w-full"
                      >
